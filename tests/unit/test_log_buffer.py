...
async def acollect(stream: AsyncIterator[str], limit: int) -> list[str]:
    result: list[str] = []
...
@pytest.mark.asyncio
async def test_catch_up_and_live_stream_delivery() -> None:
    buffer = LogBuffer()
...
@pytest.mark.asyncio
async def test_done_terminates_active_stream() -> None:
    buffer = LogBuffer()
...
@pytest.mark.asyncio
async def test_clear_resets_buffer_and_closes_stream() -> None:
    buffer = LogBuffer()
